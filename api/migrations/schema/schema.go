...
// River's own schema (D-L). It runs over MIGRATIONS_DATABASE_URL, under
// this set's own advisory lock -- distinct from the bootstrap set's lock
// id so the two sets serialize independently of one another.
//
...
// object, never to authenticate directly. MIGRATIONS_DATABASE_URL
// therefore connects as a role that can `SET ROLE vecingest_owner`
// (a superuser can always do this regardless of role membership), and
// every migration in this set issues that SET ROLE as its first
// statement. This is structurally required, not cosmetic: the bootstrap
// set's `ALTER DEFAULT PRIVILEGES FOR ROLE vecingest_owner GRANT SELECT,
// INSERT ON TABLES TO app_rw` (D-B) only fires for objects actually
// *created by* vecingest_owner -- a table created by a different role
// gets no default privileges from that ACL entry at all, which would
// silently defeat the fail-closed baseline.
package schema
